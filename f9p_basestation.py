...
def config_rtcm(port_type: str) -> UBXMessage:
    ...
    layers = 4  # 1 = RAM, 2 = BBR, 4 = Flash (can be OR'd)
    transaction = 0
    cfg_data = []
    for rtcm_type in (
        "1005",
...
def settings(port, baud, limit, duration):
    ...
    ACC_LIMIT = limit  # accuracy in mm

    # only used if TMODE = 1 ...
    SVIN_MIN_DUR = duration  # seconds
...
if __name__ == "__main__":
    settings(port="/dev/ttyACM0", baud=115200, limit = 12000, duration = 180)